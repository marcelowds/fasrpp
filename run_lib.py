...
def evaluate(config, workdir, eval_folder="sr_results", part=None):
  ...
  if os.path.isfile(ckpt_filename):
    print(f"Arquivo de checkpoint encontrado em: {ckpt_filename}")
  else:
    print(f"NÃO há arquivo de checkpoint em: {ckpt_filename}")
  ...
  total_path_features = config.data.features_path 
  with open(total_path_features,'rb') as f:
    features_media = torch.load(f)
  print(f'Path features: {total_path_features}')
  print(features_media.shape)
  ...
  for i, batch in enumerate(sample_iter):
    ...
    print(f"Starting batch {i+1} of {num_iter}")
    ...
    for j in range(2):
      ...
      output_sr_list.append(output_sr)

      manipule.save_ensemble_j(
        output_sr,
        sample_dir,
        '-sr.png',
        batch_size,
    ...
    # Média das duas SR
    output_sr_mean = torch.stack(output_sr_list, dim=0).mean(dim=0)

    # Salva uma imagem média por identidade
    manipule.save_img(
      output_sr_mean,
      sample_dir,
      '-sr_mean.png',
    ...
    print(
        ...
        f"{i * batch_size} to {(i + 1) * batch_size - 1}"
    )
